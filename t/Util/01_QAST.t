...
{ # dump ----------------------------------------------------------------------
    ...
    my $null_s := QAST::Op.new(:op<null_s>);
    is(dump($null_s), '──null_s', 'dump Op null_s');
    $null_s.returns(str);
    is(dump($null_s), '──null_s :returns(str)', 'dump Op null_s with explicit :returns');
    ...
    $v.returns(str);
    is(dump($v), '─○ local foo :returns(str)', 'dump Var with explicit :returns and scope "local"');

    $v.returns(NQPMu); # undo explicit :returns
    $v.scope('lexical');
    is(dump($v), '─○ foo', 'dump Var with explicit scope "lexical"');

    $v.returns(int);
    is(dump($v), '─○ foo :returns(int)', 'dump Var with explicit :returns and scope "lexical"');
    ...
    my $bind := QAST::Op.new(:op<bind>,
        $v,
        QAST::SVal.new(:value<bazinga!>)
    );
    is(dump($bind),
                 "──bind"
             ~ "\n  ├○ foo"
             ~ "\n  └◙ SVal \"bazinga!\"", 
        'dump Op bind with children uses single vertical line to connect children');

    $bind.returns($bind[1].returns);
    is(dump($bind),
                 "──bind :returns(str)"
             ~ "\n  ├○ foo"
             ~ "\n  └◙ SVal \"bazinga!\"", 
        'dump Op bind with :returns propagated from value');
...
{ # collect_params_and_body ---------------------------------------------------
    ...
    lives_ok( { $out := collect_params_and_body($ast) }, 'collect_params_and_body on Block with named param') 
        || diag(dump($ast));
    ...
    isa_ok($out<locals>{'$x'}, QAST::Var, :name('$x'), :decl<var>, 'collect_params_and_body on Block with local Var decl collects local decls under <locals>')
        || diag($out);
    isa_ok($out<body>, QAST::Block, :blocktype<immediate>, 'collect_params_and_body on Block with local Var decl returns an immediate Block as <body> (a)')
        || diag($out);
...
{ # isOp ----------------------------------------------------------------------
    ...
    dies_ok({ isOp(Foo.new, 'list')}, 'isOp on Foo instance with op-name');
}
...
{ # isIVal --------------------------------------------------------------------
    is(isIVal(QAST::SVal.new(:value<foo>)),     0, 'isIVal on QAST::SVal instance');
    is(isIVal(QAST::IVal.new(:value(471))),     1, 'isIVal on QAST::IVal instance');
    is(isIVal(QAST::NVal.new(:value(4.1))),     0, 'isIVal on QAST::NVal instance');
...
{ # isIVal --------------------------------------------------------------------
    is(isNVal(QAST::SVal.new(:value<foo>)),     0, 'isNVal on QAST::SVal instance');
    is(isNVal(QAST::IVal.new(:value(471))),     0, 'isNVal on QAST::IVal instance');
    is(isNVal(QAST::NVal.new(:value(4.1))),     1, 'isNVal on QAST::NVal instance');
